...
const USER_AUTH_FILE = path.join(__dirname, 'user_auth.json'); // ✨ 사번별 PIN 해시 저장 (평문 저장 금지)
const SESSION_FILE = path.join(__dirname, 'sessions.json');    // ✨ 로그인 세션 토큰 저장

let memoryLogs = [];
let memoryTokens = null;
let memorySessions = null; // { token: { userId, userName, createdAt } }
...
// ============================================================
// ✨ 사용자 인증 (사번 + PIN, 서버 검증)
// ⚠️ 기존에는 프론트엔드 USER_MAP + localStorage로만 "인증"을 흉내냈으나,
//    이는 누구나 개발자도구로 사번 목록을 보거나 localStorage를 직접 조작해
//    다른 사람 이름으로 접속할 수 있는 심각한 허점이었다.
//    이제는 사번+PIN을 서버가 검증하고, 성공 시에만 세션 토�큰을 발급한다.
//    PIN은 절대 평문으로 저장하지 않고 scrypt로 해시해서만 저장한다.
// ============================================================

// PIN 해시 생성 (salt + scrypt)
...
function verifyPin(pin, stored) {
  ...
  const inputHash = crypto.scryptSync(String(pin), salt, 64).toString('hex');
  // 타이밍 공격 방지를 위해 길이 우선 비교 후 timingSafeEqual 사용
...
// 사번 → { name, pinHash } 매핑. Render 환경에서는 파일이 재배포마다 초기화되므로
// USER_AUTH_SEED 환경변수(JSON 문자열: {"225298":{"name":"김양섭","pin":"1234"}, ...})로
// 최초 1회 시딩하고, 이후 /api/auth/set-pin 으로 각자 PIN을 변경하면 파일에 저장된다.
function loadUserAuth() {
  if (memoryUserAuth) return memoryUserAuth;
  try {
    if (fs.existsSync(USER_AUTH_FILE)) {
      memoryUserAuth = JSON.parse(fs.readFileSync(USER_AUTH_FILE, 'utf8'));
      return memoryUserAuth;
    }
  } catch (error) {
    console.error('사용자 인증 파일 읽기 실패:', error.message);
  }

  // 파일이 없으면 환경변수 시드로 최초 생성
  const seedRaw = process.env.USER_AUTH_SEED;
  if (seedRaw) {
    try {
      const seed = JSON.parse(seedRaw);
      const auth = {};
      Object.entries(seed).forEach(([empId, info]) => {
        auth[empId] = { name: info.name, pinHash: hashPin(info.pin) };
      });
      memoryUserAuth = auth;
      saveUserAuth(auth);
      console.log(`✅ USER_AUTH_SEED로 사용자 인증 정보 ${Object.keys(auth).length}건 초기화`);
      return auth;
    } catch (error) {
      console.error('❌ USER_AUTH_SEED 파싱 실패:', error.message);
    }
  }
...
function saveUserAuth(auth) {
  memoryUserAuth = auth;
  try {
    fs.writeFileSync(USER_AUTH_FILE, JSON.stringify(auth, null, 2));
  } catch (error) {
    console.error('❌ 사용자 인증 파일 저장 실패:', error.message);
  }
}
...
const SESSION_DURATION_MS = 12 * 60 * 60 * 1000; // 12시간 유효

function createSession(empId, userName) {
  const sessions = loadSessions();
  const token = crypto.randomBytes(32).toString('hex');
  sessions[token] = { empId, userName, createdAt: Date.now() };
...
function requireAuth(req, res, next) {
  ...
  req.authUser = session; // { empId, userName }
  next();
}
...
// ============================================================
app.post('/api/inventory/add-part', async (req, res) => {
  try {
...
app.post('/api/inventory/delete-part', async (req, res) => {
  try {
